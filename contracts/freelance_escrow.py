...
def _extract_date(body: str) -> str:
    """Pull a YYYY-MM-DD out of a JSON body from any of the time sources."""
    try:
        data = json.loads(body)
    except Exception:
        return ""
    for key in ("datetime", "utc_datetime", "dateTime", "currentDateTime"):
        val = data.get(key) if isinstance(data, dict) else None
        if isinstance(val, str) and len(val) >= 10:
            head = val[:10]
            if _parse_date(head) != -1:
                return head
    return ""
...
def _fetch_today() -> str:
    for url in _TIME_URLS:
        got = _extract_date(_safe_fetch(url, 2000))
        if got:
            return got
    return ""


def _today_utc_soft() -> str:
    """Consensus UTC date, or "" when no time source is reachable."""
    value = gl.eq_principle.strict_eq(_fetch_today)
    if not value or len(value) != 10 or _parse_date(value) == -1:
        return ""
    return value


def _today_utc() -> str:
    """Consensus UTC date (YYYY-MM-DD); raises when unavailable."""
    value = _today_utc_soft()
    if not value:
...
class FreelanceEscrow(gl.Contract):
    ...
    def submit_work(
        ...
        stamped = _today_utc_soft()
        if not stamped:
            # Time oracles unreachable — fall back to the client-reported
            # timestamp so a submission is never blocked by an outage.
            stamped = (submitted_at or "")[:10]
            if _parse_date(stamped) == -1:
                stamped = ""
        job["submitted_date"] = stamped
    ...
    def open_dispute(self, job_id: str) -> None:
        ...
        job["status"] = "disputed"
        job["dispute_opened_date"] = _today_utc_soft()
        self._save_jobs(jobs)
